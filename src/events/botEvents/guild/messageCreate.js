...
module.exports.run = async (client, message) => {
    ...
    if (message.content.match(mention)) {
        const embed = new EmbedBuilder().setColor(client.color).setDescription(`👋🏻 Hey Users\n${emoji.custom_emoji.author} Prefix Pookie Musix for this server is: \`/\` ( Slash Commands )`);
    ...
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("Vote Pookie Musix").setURL(voteUrl).setStyle(ButtonStyle.Link));
