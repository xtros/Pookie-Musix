...
module.exports.run = async (client, interaction) => {
    ...
    if (interaction.type === InteractionType.ApplicationCommand) {
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setLabel("Vote Pookie Musix").setURL(voteUrl).setStyle(ButtonStyle.Link),
        );
        ...
        // Fetch default music channel if user is not in VC
        if (!channel) {
            const MusicChannel = require("../../../settings/models/MusicChannel.js");
            const defaultVc = await MusicChannel.findOne({ guild: interaction.guild.id });
            if (defaultVc && defaultVc.channelId) {
                channel = interaction.guild.channels.cache.get(defaultVc.channelId);
                interaction.customVoiceChannel = channel;
            }
        }
        ...
        if (command.settings.inVc) {
            if (!channel) {
                warning.setDescription(`${emoji.custom_emoji.cross} | You must be in a voice channel or set a default voice channel to use this command.`);
        ...
        try {
            ...
            return interaction.editReply({ embeds: [warning], components: [row], ephemeral: true });
        }
    }
};
